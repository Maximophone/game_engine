...
class Physics2D:
    ...
    def add(self, go: GameObject):
        ...
        if rb is not None and rb.raw_body is None:
            ...
            body_def.position = b2Vec2(go.transform.position.x, go.transform.position.y)
            ...
            if circle_collider is not None:
                ...
                shape.SetAsBox(half_size.x, half_size.y, b2Vec2(origin.x, origin.y), 0)

                body_def.position = body_def.position + b2Vec2(offset)
